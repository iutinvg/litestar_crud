...
Copied from latest version of advanced-alchemy.
Using native hashing.
"""
...
class HashingBackend(abc.ABC):
    ...
    def compare_expression(
            ...
            plain: "Union[str, bytes]") -> "BinaryExpression[bool]":
...
class PasswordHash(TypeDecorator[str]):
    ...
    def process_bind_param(
            self, value: Any,
            dialect: Any) -> "Union[str, FunctionElement[str], None]":
    ...
    ) -> "Union[HashedPassword, None]":  # type: ignore[override]
    ...
    def compare_value(
            self, column: "ColumnElement[str]",
            plain_password: "Union[str, bytes]") -> "BinaryExpression[bool]":
...
class NativeHasher(HashingBackend):
    """Hashing backend using Python library."""
    ...
    def hash(self, value: "Union[str, bytes]") -> str:
        ...
        """
        pw_hash: bytes = _get_hash(value, self.salt)
        enc64: str = base64.b64encode(pw_hash).decode()
        return enc64
    ...
    def verify(self, plain: "Union[str, bytes]", hashed: str) -> bool:
        ...
        Returns:
            True if the password matches the hash, False otherwise.
        """
        hashed_b: bytes = base64.b64decode(hashed)
        pw_hash = _get_hash(plain, self.salt)
        return hmac.compare_digest(hashed_b, pw_hash)

    def compare_expression(
            self, column: "ColumnElement[str]",
            plain: "Union[str, bytes]") -> "BinaryExpression[bool]":
        """Direct SQL comparison is not supported
        """
        msg = "Hash does not support direct SQL comparison."
        raise NotImplementedError(msg)


def _get_hash(password_str: str, salt_str: str) -> bytes:
    hash_b = hashlib.pbkdf2_hmac('sha256', password_str.encode(),
                                 salt_str.encode(), 10)
    return hash_b
